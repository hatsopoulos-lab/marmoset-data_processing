...
from hatlab_nwb_functions import create_nwb_copy_without_acquisition, load_dict_from_hdf5
...
class dpath:
    ...
    reach_data = '/project/nicho/data/marmosets/processed_datasets/reach_and_trajectory_information/20230416_reach_and_trajectory_info.h5'
...
def filtered_kinematics_to_processing_module(event_data, meta, video_event_timeseries, series_name, original_videos, labeled_videos, kinematics_pm):
    ...
    for mIdx, mName in enumerate(event_data['marker_names']):
        ...
        mName = mName.decode("utf-8") if type(mName) == bytes else mName
        reproj_error = meta[f'{mName}_error'].to_numpy()  # a confidence value for every frame
...
def add_kinematic_data_to_nwb(reach_data, dlc, dlc_metadata, video_event_info, expName, session, scorer, nwb_infile, nwb_outfile):
    ...
    elif expName == 'crickets' or expName == 'moths' or expName == 'moth':
...
if __name__ == '__main__':
    ...
    reach_data = load_dict_from_hdf5(dpath.reach_data, top_level_list = True)
    
    resort_idx = [0, 1, 8, 9, 10, 11, 12, 13, 14, 15, 2, 3, 4, 5, 6, 7] # FIXME
    reach_data = [reach_data[idx] for idx in resort_idx]
