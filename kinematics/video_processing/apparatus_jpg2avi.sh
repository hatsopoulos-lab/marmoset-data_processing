...
for sNum in $sNums; do
    ...
    for cam in $cams; do
        ...
        for (( event=1; event <= lastEvent; event++ )); do
            ...
            ffmpeg -r $fps -f image2 -s 1440x1080 -pattern_type glob -i $path/jpg_cam$cam/${marms}_${date}_${exp}_session_${sNum}_cam${cam}_event_${eventNum}_frame_'*.jpg' -crf 15 -vcodec libx265 $video_path/${marms}_${date}_${time}_${exp}_s${sNum}_e${eventNum}_cam$cam.avi        
        done
    done
done
