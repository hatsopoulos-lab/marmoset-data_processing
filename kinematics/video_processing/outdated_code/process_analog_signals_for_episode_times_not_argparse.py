...
def get_filepaths(ephys_path, kin_path, marms_ephys_code, marms_kin_code, date_start, date_end):
    ...
    if date_end is not None:
        ephys_folders = [fold for fold in ephys_folders 
                         ...
                         and int(re.findall(datePattern, os.path.basename(fold))[0]) <= int(date_start)
...
def timestamps_to_nwb(nwbfile_path, kin_folders, saveData, batchjob_experiment):
    ###### TO NWB ######
    ...
    while not opened:
        try:
            with NWBHDF5IO(nwbfile_path, 'r+') as io:
                ...
                timestamps_module_name = 'episode_timestamps_%s' % batchjob_experiment
                timestamps_module_desc = 'set of timeseries holding timestamps for each behavior/video episode for experiment = %s' % batchjob_experiment
                ...
                dropframes_module_name = 'dropped_frames_%s' % batchjob_experiment
                ...
                episodes_intervals_name = 'episodes_%s' % batchjob_experiment
                if episodes_intervals_name in nwbfile.intervals.keys():
                    ...
                    episodes_df = nwbfile.intervals['episodes_test'].to_dataframe()                    
                else:
                ...
                for frame_times, event_info, exp_name in zip(saveData['frameTimes_byEvent'], saveData['event_info'], saveData['experiments']):
                    if exp_name != batchjob_experiment:
                        continue
...
#     task_id = int(os.getenv('SLURM_ARRAY_TASK_ID'))
    ...
    neural_processing_code_path = '/project/nicho/projects/marmosets/code_database/data_processing/neural'
    touchscreen_path = []
    touchscreen = False
    ephys_path = '/project/nicho/data/marmosets/electrophys_data_for_processing'
    kin_path   = '/project/nicho/data/marmosets/kinematics_videos'
    
    marms_ephys_code = 'TY' #'TY' # could be TY, JL, TYJL, etc - look in electroPhys_archive to find this info and to find the start date to use
    marms_kin_code = 'TYJL' #check in kinematics_videos directory
    ...
    nsx_filetype = 'ns6'
    batchjob_experiment = 'test'
    
    ephys_folders, kin_folders = get_filepaths(ephys_path, kin_path, marms_ephys_code, marms_kin_code, date_start, date_end)    
    ...
    for eFold in ephys_folders:
        ...
        if touchscreen:
            ts_trialData = load_touchscreen_data(touchscreen_path, date)
        ...
        for nsx_path in analogFiles:
            ...
            subprocess.call(['python',
                             os.path.join(neural_processing_code_path, 'create_nwb_and_store_ns6_data.py'),
                             '-f', nsx_path])
            timestamps_to_nwb(nwbfile_path, kin_folders, saveData, batchjob_experiment)