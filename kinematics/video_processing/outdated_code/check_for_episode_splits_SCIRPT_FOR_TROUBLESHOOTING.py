...
def find_and_correct_splits_in_all_episodes(jpg_dir,
    ...
    for sIdx, sNum in enumerate(session_nums):
        ...
        fix_episode_numbers(event_frame_nums, 
                            ncams, 
                            cam_timestamps, 
                            cam_event_image_files, 
                            event_start_times, 
    ...
    episode_adjustment_records = {'original_episode_frame_counts'  : frame_nums_df_original,
                                  'original_episode_start_times'   : start_times_df_original,
                                  'corrected_episode_frame_counts' : frame_nums_df,
                                  'corrected_episode_start_times'  : start_times_df} 
...
if __name__ == '__main__':
    ...
    for date in dates:
        data_path = os.path.join(vid_dir, '%s/%s/%s' % (exp_name, marms, date))
        episode_correction_record_path = os.path.join(data_path, 'episode_correction_records')
        unfiltered_videos_path         = os.path.join(data_path, 'unfiltered_videos')
        os.makedirs(episode_correction_record_path, exist_ok=True)
    
    
        find_and_correct_splits_in_all_episodes(jpg_dir,
                                                vid_dir,
                                                marms, 
                                                date, 
                                                exp_name, 
