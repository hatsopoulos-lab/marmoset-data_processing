...
def processVideo(vidPath):
    ...
    path, infile = os.path.split(vidPath)
    outfile = os.path.join(path, 'filtered_avi_videos', infile.split('.avi')[0] + '_filtered.avi')
    cap.release()
...
if __name__ == '__main__':
    ...
    ap.add_argument("-p", "--nProc", default=14,
     	help="how many processes to use. Match this with nodes*ppn from the .pbs job script")
    ...
    for ddir in date_dirs:
        os.makedirs(os.path.join(ddir, 'filtered_avi_videos'), exist_ok=True)       
        videos.extend(sorted(glob.glob(os.path.join(ddir, '*.avi'))))
