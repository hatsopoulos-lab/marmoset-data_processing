...
marmoset_ephys_code = 'JL' #'TY' # could be TY, JL, TYJL, etc - look in electroPhys_archive to find this info and to find the start date to use
marmoset_kinematics_code = 'TYJL' #check in kinematics_videos directory
date_start = '20220412' # format is YYYYMMDD
    ...
    for expNum, (exp, fCounts) in enumerate(zip(expNames, allExp_frameCounts)):
        ...
        for sess in range(numSessions):
            ...
            for eventNum in range(evTimes.shape[-1]):
                ...
                event_info.iloc[evCount, :] = [exp, uniqueSessions[sess], video_session, start_time, end_time]
