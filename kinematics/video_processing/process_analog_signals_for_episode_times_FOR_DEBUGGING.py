        ...
        try:
            with NWBHDF5IO(nwbfile_path, 'r+') as io:
                ...
                for frame_times, event_info, exp_name in zip(saveData['frameTimes_byEvent'], saveData['event_info'], saveData['experiments']):
                    ...
                    timestamps_module_desc = 'set of timeseries holding timestamps for each behavior/video episode for experiment = %s' % exp_name
...
def wait_for_all_batch_jobs_to_finish_video_conversion(matched_kinFolders):
    ...
    while not all([os.path.exists(vPath) for vPath in video_paths]):
        print('At least one experiment does not yet have the avi_videos path. Looping for another %d minutes. Time elapsed = %d minutes' % (int(time_to_wait / 60), int(stopwatch/60)), flush=True)
        time.sleep(time_to_wait)
        stopwatch += time_to_wait

    time.sleep(time_to_wait)

    print('internal_check 1', flush=True)
    ...
    while not all(videos_done):
        ...
        print('At least one experiment is still adding videos to avi_videos. Looping for another %d minutes. Time elapsed = %d minutes' % (time_to_wait / 60, stopwatch/60), flush=True)
        time.sleep(time_to_wait)
        stopwatch += time_to_wait
...
if __name__ == '__main__':
    ...
    task_id = 0
    touchscreen = False
    args = {'ephys_path'     : '/project/nicho/data/marmosets/electrophys_data_for_processing',
