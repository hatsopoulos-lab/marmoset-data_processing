...
def compute_pose_with_anipose(anipose_args):  
    ...
    if not anipose_args['only_3D']:        
        ...
        if anipose_args['task_id'] == 0:
            os.chdir(anipose_args['date_dir'])
            while len(glob.glob(pjoin(anipose_args['date_dir'], 'pose-2d-viterbi', '*'))) < len(glob.glob(pjoin(anipose_args['date_dir'], 'avi_videos', '*'))):
                time.sleep(10)
...
if __name__ == '__main__':
    ...
    for date in args['dates']:
        ...
        task_ep_cutoffs = np.ceil(np.linspace(events[0], events[-1]+1, n_tasks+1))   
        task_ep_cutoffs = [int(cut) for cut in task_ep_cutoffs]
        task_ep_range   = range(task_ep_cutoffs[task_id], task_ep_cutoffs[task_id+1])
        task_videos     = [vid for vid in videos if int(re.findall(epPattern, os.path.basename(vid))[0].split('e')[-1]) in task_ep_range]   
