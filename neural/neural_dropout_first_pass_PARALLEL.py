...
def get_filepaths(ephys_path, kin_path, marms_ephys_code, marms_kin_code, date):
    ...
    print('ephys folders:', ephys_folders)
...
def identify_dropout(filepath, binwin, dropout_method = 'spikes', plot = False):
    ...
    with NWBHDF5IO(filepath, 'r+') as io:
        ...
        print(f"Fraction of Bins dropped, in seconds: {(len(val)-sum(val))*data['binwin']}s out of {len(val)*data['binwin']}s", flush=True)
...
if __name__ == '__main__':
    ...
    if not debugging:
        ...
        args = {'kin_dir' : '/project/nicho/data/marmosets/kinematics_videos',
                'ephys_path' : '/project/nicho/data/marmosets/electrophys_data_for_processing/JLTY20241029_1200_baseline',
                'date' : '2024_10_29',
                'marms': 'TYJL',
                'marms_ephys': 'TYJL',
                'exp_name':'baseline',
